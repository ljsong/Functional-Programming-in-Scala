...
import scala.collection.par.Conc.Single
...
package object barneshut {
  ...
  extends Quad {
    val (mass, massX, massY) = (bodies.map(_.mass).sum : Float,
      bodies.map((b: Body) => b.x * b.mass).sum / bodies.map(_.mass).sum : Float,
      bodies.map((b: Body) => b.y * b.mass).sum / bodies.map(_.mass).sum : Float)
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val xval: Int = ((b.x - boundaries.minX) / sectorSize).toInt
      val yval: Int = ((b.y - boundaries.minY) / sectorSize).toInt
      val ix =
        if (xval < 0) 0
        else if (xval >= sectorPrecision) sectorPrecision - 1
        else xval
      val jx =
        if (yval < 0) 0
        else if (yval >= sectorPrecision) sectorPrecision - 1
        else yval
      this(ix, jx) += b
      this
    }
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      for (ix <- matrix.indices) matrix.update(ix, matrix(ix).combine(that.matrix(ix)))
      this
    }
