...
class KMeansSuite extends FunSuite {
  ...
  test("'classify' should work for 'points' == GenSeq((0, 0, 1), (0, 0, -1), (0, 1, 0), (0, 10, 0)) and 'means' == GenSeq((0, -1, 0), (0, 2, 0))") {
    ...
    val p6 = new Point(0, 5.5, 0)
    val newMeans: GenSeq[Point] = IndexedSeq(p5, p6)
    assert(kMeans(points, means, eta) == newMeans, s"kMeans($points, $means, $eta) should equal to $newMeans")
  }
